...
public class AuthController {
    ...
    @RolesAllowed({"ADMIN", "EMPLOYEE"})
    public ResponseEntity<OperationResponse> registerUser(@RequestBody @Valid RegistrationRequest inputUser)
            throws URISyntaxException {
        User savedUser = authService.registerUser(inputUser);
        return ResponseEntity.created(new URI("/api/operations/" + savedUser.getId())).body(
                new OperationResponse("New user with id: " + savedUser.getId() + " and login: " + savedUser.getLogin() +
                        " registered."));
    }

    @PostMapping("/register-with-account")
    @RolesAllowed({"ADMIN", "EMPLOYEE"})
    public ResponseEntity<OperationResponse> registerUserWithAccount(@RequestBody @Valid RegistrationRequest inputUser)
            throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/operations/" + user.getId())).body(new OperationResponse(
                "New user with id: " + user.getId() + " and login: " + user.getLogin() + " registered. " +
                        "New card is created with id: " +
